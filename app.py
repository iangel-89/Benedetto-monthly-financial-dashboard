...
def render_take_home(M, statements, entity, audit, api_key, model_override):
    ...
    dashboard's sections, with chart images matching what's on screen and —
    when possible — the AI executive summary. Always visible in the main
    frame, regardless of which dashboard view (Simple/Detailed) is active.
        ...
        with st.spinner("Building your report…"):
            ...
            if st.session_state.get("ai_summary_key") == ai_key and st.session_state.get("ai_summary_text"):
                ...
                try:
                    ...
                    st.session_state["ai_summary_key"] = ai_key
                except AIError as e:
            ...
            st.session_state["xlsx_key"] = xlsx_key
            st.session_state["xlsx_has_ai"] = bool(ai_text)
            st.session_state["xlsx_note"] = note

    if st.session_state.get("xlsx_bytes") and st.session_state.get("xlsx_key") == xlsx_key:
        ...
        st.download_button(
            f"⬇ Download Excel report — {export_view} ({badge})",
            data=st.session_state["xlsx_bytes"],
            file_name=f"{entity.replace(' ', '_')}_Financial_Report_{export_view}.xlsx",
